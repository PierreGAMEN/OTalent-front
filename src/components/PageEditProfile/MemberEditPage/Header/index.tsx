        ...
        <div className='container-header-EditProfilPage'>
            ...
            <div className=''>
                ...
                (<>
                    ...
                    <button onClick={()=> {setIsAddNewCategory(!isAddNewCategegory)}}>{!isAddNewCategegory ? "Ajouter une nouvelle catégorie" : "Ne plus ajouter de catégorie"}</button>
                    {isAddNewCategegory && <>
                    <select
                ...
                {selectedCategory && <button onClick={AddCategory}>V</button>}
                  </>
                    }
                    <div>{favoriteCategories.map(categorie => (<p key={categorie.id}><button onClick={(e) => {deleteCategorie(e)}} id={categorie.id}>X</button>{categorie.label}</p>))}</div>
                </> ) : (<>
                    <p>Catégories préférées :</p>
                    <div>
