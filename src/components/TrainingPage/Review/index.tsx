...
export default function ReviewTrainingPage ({ data }: { data: TrainingDataI }) {
    ...
    return (<section className="reviews-section">
        ...
        <button onClick={openModalAddComment} className="reviews-section-button">Je donne mon avis</button>
        ...
        <div className="modal-add-comment">
            ...
            <label className="modal-add-comment-input-label" htmlFor="note">Attribuez une note : </label>
            <select 
            value={selectedNote}
            ...
            <div className="modal-add-comment-input">
            <label className="modal-add-comment-input-label" htmlFor="comment">Commentaire</label>
            <textarea onChange={(e) => onChangeComment(e)} value={comment} placeholder="Notez votre commentaire ici" name="comment" id="" cols="40" rows="10"></textarea>
            </div>

            <div className="modal-add-comment-button">
            <button onClick={addComment}className="modal-add-comment-button-button">Ajouter le commentaire</button>
            <button className="modal-add-comment-button-button" onClick={() => {setModalAddCommentIsOpen(false)}}>Fermer l'espace commentaire</button>
            </div>
        ...
        {currentComment.length < 1 && <p>Il n'y a pas encore de commentaire sur cette formation</p>}
