  ...
  const getUserInformation = async () => {
    try {
      ...
      if (user.member === false) {
        ...
        dispatch(
          getOrganizationInformationActions(organizationInformationDispatch)
        );
      }
      ...
      <div className="hidden lg:inline-block">
        {isMember && (
          <p className="text-white p-5 hidden lg:block">
            {userInformation.member && userInformation.member.firstname}
          </p>
        )}
        {!isMember && (
          <p className="text-white p-5">
            {userInformation.organization && userInformation.organization.name}
          </p>
        )}
      </div>
      <div
        tabIndex={0}
        role="button"
        className="btn btn-ghost btn-circle avatar w-16 h-16 border-4 border-white hover:border-green-600"
      >
        <div className="rounded-full">
          {isMember && (
            <img
              ...
              src={
                userInformation.member && userInformation.member.avatar
